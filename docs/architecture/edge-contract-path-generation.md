...
このメモは、imagegen由来のアーキテクチャ図、システム構成図、ワークフロー図を新規作成
するとき、および今後ノード配置・カードサイズ・フレーム位置・相関矢印ルートを改修するとき
の相関矢印生成方式を定義する。現行の
`retail-ai-ops-copilot-architecture.html` は、source画像への見た目再現を優先して
固定座標を手で詰めた状態なので、この方式へ一括自動移行しない。ただしこの例外は、
今後のレイアウト影響改修でも固定座標を無条件に増やしてよい、という意味ではない。

## 目的

相関矢印はノード移動やカードサイズ変更のあとに、始点・終点が虚空に残る、ノード内へ
食い込む、双方向矢印が片方向に見える、といった問題が出やすい。新規図またはレイアウト
影響のある改修では、`from: Semantic KPI Model / anchor: bottom` のような edge-contract を
先に定義し、SVG path の `d` はノード矩形の実測値から生成する。

## 適用範囲

- 新規の構成図、ワークフロー図、アーキテクチャ図
- 既存図でノード移動、カードサイズ変更、フレーム移動、カテゴリ枠調整、矢印ルート変更を行う改修
- 既存図で新しいノードや相関矢印を追加する改修

現行図の全pathを一括移行する必要はない。ただし既存図を改修するときは、変更対象周辺の
edgeを contract 管理へ寄せるか、source忠実化・バス・レーン・ラベルアンカーなどの理由で
固定座標例外として残すかを明示する。
...
- 既存図のノード移動、カードサイズ変更、フレーム移動、矢印ルート変更でも、この方式を適用対象として扱う
- ノード移動、カードサイズ変更、フレーム移動のあとに、contract管理edgeは該当edgeだけでなく全edgeを再生成する
...
- 現行図を全面的にこの方式へ移行する場合は、見た目差分が大きく出る可能性があるため、別タスクとしてHITL承認を取る
