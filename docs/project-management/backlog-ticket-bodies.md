...
RAIOPS-3ではdbt scaffoldと97 data testsの定義、parse/compileまでは確認済みだが、live dbt build/testはまだ通していない
したがって、RAIOPS-4では「test定義がある」状態と「Snowflake上でtestが通った」状態を分け、後段readinessへ反映する条件を固定する

作業内容:
- `docs/architecture/raiops-4-dbt-test-readiness-gate.md` をRAIOPS-4設計正本として使う
- source/staging/intermediate/martごとのtest categoryを確認する
- `mart_retail_monthly_kpi` の固定grain unique testをlive test gateに含める
- `net_sales >= 0`, `order_count > 0`, `avg_discount between 0 and 1` などの品質testをlive test gateに含める
- KPI定義seedのtest方針を決める
- `order_count` の上位粒度単純合算、率系KPIの平均を防ぐreaggregation guardをdbt singular testまたはGolden Evalへ割り当てる
- dbt test失敗時にSemantic/Eval/readinessを上げない運用をreadiness rubricへ反映する
- credentials-free parse/compileとlive build/testを証跡上分離する
- 並列sessionへ渡す場合の編集可能ファイル、触らない共有ファイル、検証コマンドを明記する

受入基準:
- `docs/architecture/raiops-4-dbt-test-readiness-gate.md` にLevel 0からLevel 3までのgateが明記されている
- 現在のparse/compile証跡がlive dbt test成功として扱われていない
- `dbt test` の対象と目的がschema.ymlまたはdocsから追える
- mart grain破壊を検出するtestがある
- dbt testsが失敗した時、Semantic/Eval/readinessを上げない運用になっている
- CIまたはlocal live実行で、credentialsなしの成功をlive build/test成功と誤認しない
- diagram tooltipまたはreadiness文書にdbt testsの検証状態を反映する条件が書かれている
- 並列sessionに渡すdbt品質gate briefがある

証跡:
- repo path: `docs/architecture/raiops-4-dbt-test-readiness-gate.md`
...
- reaggregation guardをdbt singular testで見るかGolden Evalで見るか
```
