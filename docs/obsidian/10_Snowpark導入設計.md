...
このメモは、現在のシステム構成図を Snowpark 導入版へ更新するための設計メモ。

## 結論

今回の構成図では、Snowpark を `Snowpark Python / UDF / SP` として Snowflake Account 内に追加する。

Snowpark は Cortex の代替ではなく、Snowflake 内で Python 処理、UDF、Stored Procedure、dbt Python model、eval runner、trace enrichment を動かす実行面として扱う。

SPCS は今回の主線ノードにはしない。Airflow、JupyterLab、MLflow、custom API、長時間ジョブなどを Snowflake 境界内でコンテナ実行したい段階で追加検討する。

## 構成図に出す相関関係

| 関係 | 図での表現 | 意味 | 実装前メモ |
| --- | --- | --- | --- |
| dbt tests / dbt Python -> Snowpark Python | `dbt Python / UDF / SP` | SQL/dbtだけで扱いにくいPython変換やUDF/SPをSnowflake内で実行する候補 | dbt本体が未実装なので readiness は 0% のまま |
