...
Snowpark / SPCS は、初級編の完了を止める必須ノードにはしない。ただし公開情報上は現実的な選択肢であり、中級編以降では有力候補として積極的に検討する。SQL/dbtだけでは表現しにくいPython処理、UDF/SP、Snowflake内eval runner、trace enrichment、ML feature、Container Services上のAirflow/Jupyter/MLflow/custom APIを扱う段階で、意識的に採用候補へ上げる。
