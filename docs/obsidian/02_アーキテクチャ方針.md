...
```mermaid
flowchart LR
  TPCH["TPCH_SF1 current sample"] --> MartBuild["SQL/dbt mart build"]
  FutureSamples["Tasty Bytes / richer retail samples later"] --> MartBuild
...
Snowpark / SPCS は、初級編の完了を止める必須ノードにはしない。ただし公開情報上は現実的な選択肢であり、中級編以降では有力候補として積極的に検討する。現構成図ではまず `Snowpark Python / UDF / SP` を Snowflake Account 内の実行面として追加し、dbt Python model、Cortex Agents custom tool、eval runner、trace enrichment の候補として扱う。SPCS は Container Services 上の Airflow/Jupyter/MLflow/custom API を扱う段階で追加検討する。
...
- Snowpark / SPCS は複雑前処理、ML、Snowflake内Python、コンテナ化サービスで使われうるが、全案件標準とは断定しない
