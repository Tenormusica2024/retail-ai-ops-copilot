...
このプロジェクトは初級編の学習面として扱う。1ヶ月程度の短い学習期間では、初級編の完成度を上げ続けるより、Cortex/dbt/Streamlit/LLMOpsの基本関係を早く押さえ、中級編以降の難しい構成へ進む方が学習効率が高い。
...
中級編をこのrepoの拡張にするか、別プロジェクトに分けるかは未決定。開始前に壁打ちで判断する。
